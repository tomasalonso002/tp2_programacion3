  ...
  return (
    ...
    {loading ? (
      ...
      {selectedGame ? (
        ...
        {selectedGame.released ? new Date(selectedGame.released).toLocaleDateString() : "No disponible"}
        </p>
        <p>
        <span className="font-semibold">Rating:</span>{" "}
        ⭐ {selectedGame.rating}/5 ({selectedGame.ratings_count || 0} votos)
        </p>
        <p>
        <span className="font-semibold">Géneros:</span>{" "}
        {selectedGame.genres?.map(g => g.name).join(", ") || "No especificado"}
        </p>
        <p>
        <span className="font-semibold">Plataformas:</span>{" "}
        {selectedGame.platforms?.map(p => p.platform.name).join(", ") || "No especificado"}
        </p>
